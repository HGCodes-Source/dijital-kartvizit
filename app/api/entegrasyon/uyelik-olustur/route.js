...
export async function POST(istek) {
  ...
  var gelenAnahtar = istek.headers.get("X-Entegrasyon-Anahtari");
  var dogruAnahtar = process.env.SATIS_SITESI_ENTEGRASYON_ANAHTARI;
  ...
  // 2) Gelen veriyi oku
  var gelenVeri = await istek.json();
  var email = gelenVeri.email;
  var ad = gelenVeri.ad;
  var bitisTarihi = gelenVeri.bitisTarihi;
  var plan = gelenVeri.plan;
  ...
  var mevcutSonuc = await supabase
    .from("users")
    .select("id")
    .eq("email", email)
    .maybeSingle();
  var mevcutKullanici = mevcutSonuc.data;

  if (mevcutKullanici) {
    // MEVCUT MÜŞTERİ (yenileme): sadece paket/bitiş tarihini güncelle,
    // şifreye DOKUNMA — zaten bir şifresi var.
    var guncellemeSonucu = await supabase
      .from("users")
      .update({ plan: plan, expires_at: bitisTarihi, active: true })
      .eq("email", email);

    if (guncellemeSonucu.error) {
      console.error("Supabase güncelleme hatası:", guncellemeSonucu.error);
  ...
  var isim = ad || email.split("@")[0];
  var gecicSifre = rastgeleSifreUret();

  var eklemeSonucu = await supabase.from("users").insert({
    id: "user-" + Date.now(),
    role: "user",
    email: email,
  ...
  if (eklemeSonucu.error) {
    console.error("Supabase ekleme hatası:", eklemeSonucu.error);
  ...
  return NextResponse.json({ basari: true, yeniMi: true, gecicSifre: gecicSifre });
}